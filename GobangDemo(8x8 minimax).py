...
def evaluate_board():
    if check_win(-1):
        ...
        ai_score = sum([1 for i in range(board_size) for j in range(board_size - win_condition + 1) if all(board[i, j+jj] == -1 for jj in range(win_condition))])
        human_score = sum([1 for i in range(board_size) for j in range(board_size - win_condition + 1) if all(board[i, j+jj] == 1 for jj in range(win_condition))])
...
# Define a function to get the AI player's best move using the minimax algorithm with alpha-beta pruning
def get_ai_move():
    alpha = -np.Inf
    beta = np.Inf
    best_move = None
